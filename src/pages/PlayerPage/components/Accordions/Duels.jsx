					...
					if (Boolean(Utils.add(json[`${id}${id && '_'}wins`], json[`${id}${id && '_'}losses`]))) {
						return (
							<Row key={id} id={id} isHighlighted={id === mostPlayedMode.id}>
								...
								<Cell>{json[`current_winstreak${id && '_mode_'}${id}`] === undefined ? "?" : json[`current_winstreak${id && '_mode_'}${id}`]}</Cell>
								<Cell>{json[`best_winstreak${id && '_mode_'}${id}`] === undefined ? "?" : json[`best_winstreak${id && '_mode_'}${id}`]}</Cell>
	...
	return Utils.isEmpty(json) ?
		...
		<Accordion title={consts.TITLE} header={header} index={props.index}>
			<div className="h-flex mb-3">
				...
				<div className="flex-1">
					<Pair title="Best Winstreak">{json.best_overall_winstreak === undefined ? "?" : json.best_overall_winstreak}</Pair>
					<Pair title="Current Winstreak">{json.current_winstreak === undefined ? "?" : json.current_winstreak}</Pair>
